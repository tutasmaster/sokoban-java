...
public class SwingGamePanel extends JPanel implements KeyListener, MouseListener, MouseMotionListener, ActionListener {
    ...
    public SwingGamePanel(Game game) {
        ...
        timer.start();
        try {
            _tilemap = ImageIO.read(Map.class.getResource("assets/tilemap.png"));
    ...
    public void setEditing(Boolean b){ _is_editing = b; }
    private BufferedImage _tilemap;
    ...
    protected void paintComponent(Graphics g) {
        ...
        for (Entity e : _game.get_entity_list()){
            if (e.getClass() == Goal.class) {
                ...
                int gridPosX = (int) (p.x * scale_x);
                int gridPosY = (int) (p.y * scale_y);
        ...
        if(_is_editing && cursor != null && cursor.y < map.get_size_y() && cursor.x < map.get_size_x()){
            g.setColor(Color.WHITE);
            g.drawRect((int) (cursor.x * scale_x), (int) (cursor.y * scale_x), sX + 1, sY + 1);
        }
    }
    ...
    public void keyPressed(KeyEvent e){
        ...
        try{
            ...
            repaint();
            if(_game.goalsLeft() == 0){
                renderMessage(this, "You beat this level.", "Congratulations!");
                repaint();
            }
        }catch(Exception ex){
    ...
    public void mousePressed(MouseEvent e) {
        ...
        if(!_is_editing){
            //System.out.println(_game.get_player().findPath(cursor_pos));
            Graph g = new Graph(_game);
